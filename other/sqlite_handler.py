    ...
    if not (f'{table.name}',) in self.file_tables:
      ...
      for column in columns:
        ...
        if isinstance(column.foreign_key, SQLHTable) :
          foreign_keys_list.append(f'FOREIGN KEY({column.name}) REFERENCES {column.foreign_key.name}({column.foreign_key.primary_key.name})')
...
class SQLHTable(object):
  ...
  def get_record_by_index(self, index:int):
    records = self.get_records(rows=True)
    for record in records:
      if record[0][0] == index:
        return record
    return None
...
class SQLHColumn(object):
  def __init__(self, name:str, type:str, constraint:str = "NOT NULL", primary_key:bool = False, foreign_key:SQLHTable = False):
    self.name = name
    self.type = type
